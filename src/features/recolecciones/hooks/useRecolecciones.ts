...
export default function useRecolecciones() {
  ...
  useEffect(() => { fetchRecolecciones(currentPage, perPage); }, []);

  const onPageChange = (page: number) => { setCurrentPage(page); fetchRecolecciones(page, perPage); };
  const onPerPageChange = (pp: number) => { setPerPage(pp); setCurrentPage(1); fetchRecolecciones(1, pp); };
